 ...
 */
class PHPMailerParams implements iEmailParams {
	private $errors;
	private $validator;
	...
	function __construct() {
		$this->from = null;
		$this->validator = new PHPMailerConfig::$requires['Validator'];

		$this->errors = new PHPMailerConfig::$requires['ErrorList'](PHPMailerConfig::$availableErrors);
	...
	public function setTo($name, $email) {
		...
		if(!$result->valid) {
			$this->errors->addError(PHPMailerConfig::ERROR_INVALID_EMAIL, array(), 'to');
		}
	}
	...
	public function setFrom($name, $email) {
		...
		$result = $this->validator->validate($email, $options);
		if(!$result->valid) {
			$this->errors->addError(PHPMailerConfig::ERROR_INVALID_EMAIL, array(), 'from');
		}
	}
	...
	public function setSubject($subject) {
		...
		if(!$result->valid) {
			$this->errors->addError(PHPMailerConfig::ERROR_TOO_LONG, array(), 'subject');
		}
	}
	...
	public function setTemplatePathText($pathOrArray) {
		...
		if(!$textTemplateValid) {
			$this->errors->addError(PHPMailerConfig::ERROR_INVALID_TEXT_TEMPLATE, array(), 'templatePathText');
		}
	}
	...
	public function setTemplatePathHtml($path) {
		...
		if(!$htmlTemplateValid) {
			$this->errors->addError(PHPMailerConfig::ERROR_INVALID_HTML_TEMPLATE, array(), 'templatePathHtml');
		}
	}
